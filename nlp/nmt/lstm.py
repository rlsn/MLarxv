...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        if self.bidirectional:
            ''' In the case of bi-directional LSTM, the final hidden/cell states of the 
            two directions are concatenated (forming state vectors twice as long as before).'''
            def combine_directions(outs):
                ''' Entangle the interleaved layers by their direction and concatenate them
                along the state vector dimension.
                [num_layers * 2, batch, hidden_size] -> [num_layers, batch, hidden_size * 2]
                '''
                return torch.cat([outs[0: outs.size(0): 2], outs[1: outs.size(0): 2]], dim=2)
            ''' Perform the direction combining on both the hidden and cell states
            
            final_hidden_states (h_t): the hidden states of all layers at the final step.
            
            final_cell_states (c_t): the cell states of all layers at the final step. 
            Cell state is the key to tackling the vanishing gradient problem.'''
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        if src_mask is not None:
            ''' src_mask is a binary tensor with size [batch_size, src_time_steps] where 1's indicate paddings.
            Here we add an extra dimension to match the shape of attn_scores [batch_size, 1, src_time_steps]
            so as to constrain the attention scores of the paddings to be negative infinity (minimal attention).
            '''
            src_mask = src_mask.unsqueeze(dim=1)
            attn_scores.masked_fill_(src_mask, float('-inf'))
        # The attention weights are obtained by softmaxing attention scores as the weights has to sum up to 1.
        attn_weights = F.softmax(attn_scores, dim=-1)
        # The batch matrix-matrix product of the weights and encoder context vector.
        # bmm: ([batch_size, 1, src_time_steps],[batch_size, src_time_steps, output_dims])->[batch_size, 1, output_dims]
        attn_context = torch.bmm(attn_weights, encoder_out).squeeze(dim=1)
        # The squeezed product is then concatenated with the target hidden states at t-1
        # context_plus_hidden has size [batch_size, output_dims + input_dims]
        context_plus_hidden = torch.cat([tgt_input, attn_context], dim=1)
        # The concatenated vector is feed to a learnable linear transformation followed by a tanh nonlinearity.
        # The attention output has shape [batch_size, output_dims], consisting of the attention context vectors for a batch.
    ...
    def score(self, tgt_input, encoder_out):
        ...
        '''
        ''' The (general) attention scores are calculated using the bilinear function: A learnable linear transformation 
        is applied to the source encoding so that the source and target encodings aren't required to be in the same 
        space (different shape). The attention score is the inner product of the target and transformed source encoding at each step,
        so that to get a high attention score, the two vectors must be close in space. 

        ([batch_size, 1, output_dims],[batch_size, output_dims, src_time_steps])->[batch_size, 1, src_time_steps]
        '''
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        '''
        ''' The cached_state stores the encoder and decoder states at previous step in auto-regressive generation (i.e. test time). 
        cached_state == None at training time, and the first step in the generation (as there 's no previous states).

        input_feed feeds the source context into the model, whether it be attentional or not.
        If the attention is used, the connection also makes the decoder aware of the previous 
        alignment choice (Luong, Thang et al., 2015).
        '''
        cached_state = utils.get_incremental_state(self, incremental_state, 'cached_state')
        if cached_state is not None:
            tgt_hidden_states, tgt_cell_states, input_feed = cached_state
        else:
            '''At t=0, the decoder hidden states and cell states (tgt_hidden_states,tgt_cell_states) are initialized to 0, for each layers.
            the encoder output (input_feed) is initialized to all 0 as well. '''
        ...
        for j in range(tgt_time_steps):
            ...
            '''
            ''' At each timestep t, the output of the decoder's final layer is feed into the attention layer, 
            together with the encoder outputs. The output of the attention layer at timestep t is used to compute the 
            decoder output, and is also auto-regressively fed to the decoder at the next timestep t+1.

            The target state of the final layer is given to the attention layer because the alignment between source and target context
            relies on information from both the encoder and decoder, and the decoder is conditioned on this alignment. 
            The target states carries encoded target contextual information and is thus useful in the alignment decision.
            
            Dropout is inserted between layers to encourage better generalization and less overfitting.
            '''
            ...
            if self.attention is None:
                ...
                if self.use_lexical_model:
                    ...
                    # weighted_embedding: [batch_size, embed_dim]
                    # lexical_contexts[tgt_time_steps]: [batch_size, 1, embed_dim]
                    weighted_embedding = torch.tanh(torch.bmm(step_attn_weights.unsqueeze(1), src_embeddings.transpose(0,1))).squeeze(1)
                    lexical_contexts.append(weighted_embedding+torch.tanh(self.lexical_FFNN(weighted_embedding)))
                    # TODO: --------------------------------------------------------------------- /CUT
        ...
        if self.use_lexical_model:
            ...
            # Collect lexical contexet across time steps [batch_size, tgt_time_steps, embed_dim] and transpose
            lexical_contexts = torch.cat(lexical_contexts, dim=0).view(tgt_time_steps, batch_size, self.embed_dim).transpose(0, 1)
