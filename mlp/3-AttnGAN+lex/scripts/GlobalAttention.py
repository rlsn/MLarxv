...
def func_attention(query, context, gamma1):
    """
    query: batch x ndf x queryL    from text
    context: batch x ndf x ih x iw (sourceL=ihxiw)      from image
...
class LexicalAttention(nn.Module):
    def __init__(self):
        pass
    def forward(self,):
        pass
...
class GlobalAttentionGeneral(nn.Module):
    ...
    def forward(self, input, context):
        """
            input: batch x idf x ih x iw (queryL=ihxiw) Image encoding
            context: batch x cdf x sourceL Text encoding
        """
