...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        for img, feature_maps in zip(inputs, outputs):
            for output_channel, bias, feature_map in zip(self.kernels, self.biases, feature_maps):
                for input_channel, kernel in zip(img, output_channel):
                    feature_map += convolve2d(input_channel, kernel, mode="valid")
                feature_map += bias
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        for grads_img, grads_feature_maps in zip(grads_wrt_inputs,grads_wrt_outputs):
            for output_channel, grads_feature_map in zip(self.kernels, grads_feature_maps):
                for grads_input_channel, kernel in zip(grads_img, output_channel):
                    grads_input_channel += convolve2d(grads_feature_map, np.flip(kernel), mode="full")
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        for img, grads_output_channels in zip(inputs,grads_wrt_outputs):
            for grads_kernels, grads_output_channel in zip(grads_wrt_kernels, grads_output_channels):
                for grads_kernel, input_channel in zip(grads_kernels, img):
                    grads_kernel += convolve2d(np.flip(input_channel), grads_output_channel, mode="valid")                                                                                             
        if self.kernels_penalty is not None:
            grads_wrt_weights += self.kernels_penalty.grad(parameter=self.kernels)
...
class MaxPooling2DLayer(Layer):
    ...
    def __init__(self, input_height, input_width, size, stride):
        ...
        """
        assert (input_height - size)%stride==0 and (input_width - size)%stride==0,"The stride does not fit the input dimension"
        self.input_height = input_height
        self.input_width = input_width
        self.size = size
        self.stride = stride
    ...
    def fprop(self, inputs):
        ...
        """
        outputs = np.zeros((inputs.shape[0],inputs.shape[1],self.output_height,self.output_width))
        self.cache = np.zeros(inputs.shape,dtype=bool)
        for input_channels,output_channels,masks in zip(inputs,outputs,self.cache):
            for input_channel,output_channel,mask in zip(input_channels,output_channels,masks):
                for i in range(0,self.output_height):
                    for j in range(0,self.output_width):                        
                        input_window = input_channel[i*self.stride:i*self.stride+self.size, j*self.stride:j*self.stride+self.size]
                        output_channel[i,j]+=np.max(input_window)
                        mask[i*self.stride:i*self.stride+self.size, j*self.stride:j*self.stride+self.size]+=(output_channel[i,j]==input_window)                        
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        """
        grads_wrt_inputs = np.zeros(inputs.shape)
        for grads_input_channels,grads_output_channels,masks in zip(grads_wrt_inputs,grads_wrt_outputs,self.cache):
            for grads_input_channel,grads_output_channel,mask in zip(grads_input_channels,grads_output_channels,masks):
                for i in range(0,self.output_height):
                    for j in range(0,self.output_width):
                        mask_window = mask[i*self.stride:i*self.stride+self.size, j*self.stride:j*self.stride+self.size]
                        grads_input_channel[i*self.stride:i*self.stride+self.size, j*self.stride:j*self.stride+self.size]+=mask_window*grads_output_channel[i,j]
        return grads_wrt_inputs
