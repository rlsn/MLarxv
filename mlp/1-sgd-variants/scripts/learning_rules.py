...
class AdamLearningRule(GradientDescentLearningRule):
    ...
    def __init__(self, learning_rate=1e-3, beta_1=0.9, beta_2=0.999,
        ...
        assert beta_2 >= 0. and beta_2 < 1., 'beta_2 should be in [0, 1).'
    ...
    def reset(self):
        ...
        """
        for mom in zip(self.moms_1):
            mom *= 0.
        for mom in zip(self.moms_2):
            mom *= 0.
        self.step_count = 0
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        self.step_count += 1 # update timestep
        for param, mom_1, mom_2, grad in zip(self.params, self.moms_1, self.moms_2, grads_wrt_params):
            ...
            moms_1_hat = mom_1/(1-self.beta_1**self.step_count)
            moms_2_hat = mom_2/(1-self.beta_2**self.step_count)

            param -= (self.learning_rate * moms_1_hat / (moms_2_hat**0.5 + self.epsilon))
...
class AdamLearningRuleWithWeightDecay(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params):
        ...
        schedule_multiplier = self.learning_rate / self.initial_learning_rate # ηt
        ...
        for param, mom_1, mom_2, grad in zip(self.params, self.moms_1, self.moms_2, grads_wrt_params):
            ...
            param -= (self.learning_rate * moms_1_hat / (moms_2_hat**0.5 + self.epsilon) + self.weight_decay*param)
...
class RMSPropLearningRule(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, mom_2, grad in zip(self.params, self.moms_2, grads_wrt_params):
            ...
            param -= (self.learning_rate * grad / (mom_2**0.5 + self.epsilon))