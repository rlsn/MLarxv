...
class CosineAnnealingWithWarmRestarts(object):
    ...
    def __init__(self, min_learning_rate, max_learning_rate, total_iters_per_period, max_learning_rate_discount_factor,
        ...
        assert max_learning_rate>0
        assert total_iters_per_period>0
    ...
    def update_learning_rule(self, learning_rule, epoch_number):
        ...
        # compute last restart epoch and coressponding T_i and max_learning_rate

        last_restart_epoch, T_i = 0, self.total_epochs_per_period
        max_learning_rate = self.max_learning_rate
        while last_restart_epoch+T_i <= epoch_number:
            last_restart_epoch += T_i
            T_i = round(T_i*self.period_iteration_expansion_factor)
            max_learning_rate *= self.max_learning_rate_discount_factor

        T_curr = epoch_number - last_restart_epoch

        learning_rate_multiplier = self.min_learning_rate+0.5*(max_learning_rate-self.min_learning_rate)*(1.0+np.cos(np.pi*T_curr/T_i))
        learning_rule.learning_rate = learning_rule.initial_learning_rate*learning_rate_multiplier
